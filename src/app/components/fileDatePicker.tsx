...
type Props = {
  setReport: React.Dispatch<React.SetStateAction<ReportApiResponse | undefined>>;
  spot: string;
  setPickerDate: React.Dispatch<React.SetStateAction<string | undefined>>;
...
export default function FileDatePicker({ setReport, spot, setPickerDate, setTemplate }: Props) {
  const [pickDate, setPickDate] = useState(new Date());
  const [isStart, setIsStart] = useState(false);
  const pickDateFormated = formatDate(pickDate);
  const fileRequested = `informe_${spot}_${pickDateFormated}.json`;

  const checkChecksum = (_report: any[]) => {
    ...
    return _report[0].metaData.checksum === checksum;
  };
  ...
  const getReport = useCallback(
    (_fileNameReq: string) => {
      ...
      FileApi.downloadjson(_fileNameReq)
        .then((res) => {
          if (res.data[0].metaData.isComplete && !checkChecksum(res.data)) {
          ...
          setReport(res as ReportApiResponse);
        })
  ...
  return (
    <div className="flex">
      <DatePicker id="calendar" locale="es" selected={pickDate} onChange={(date) => date && setPickDate(date)} dateFormat="dd-MM-yy" popperPlacement="top" />
      <button type="button" className="button" onClick={() => getReport(`informe_${spot}_${formatDate(pickDate)}.json`)}>
        ▶️
      </button>
    </div>
  );
